port module Page.Verified exposing (Model, Msg, init, route, subscriptions, update, view)
...
import Html exposing (Html, a, br, div, h1, input, label, option, select, span, text)
import Html.Attributes exposing (autocomplete, class, disabled, download, for, href, id, placeholder, rel, style, target, type_, value)
import Html.Events
    ...
    case msg of
        ...
        UserStatusChanged json ->
            case Session.decode json of
                ...
                Nothing ->
                    let
                        maybeNull =
                            Decode.decodeString (Decode.null "not-signed-in") (Encode.encode 0 json)
                    in
                    case maybeNull of
                        Ok err ->
        ...
        GetUserInfoSucceeded json ->
            case User.decode json of
                Just content ->
                    let
                        submittedContent =
                            Content content.firstName content.lastName content.degree content.terms

                        newCheckedRules =
                            if Terms.toBool content.terms then
                                ( True, True, True )
                        ...
                        newTicketStage =
                            if content.submittedTicket then
                                Created
        ...
        UpdateUserInfo session content ->
            let
                ...
                message =
                    Content.encode session newContent
            in
            ( { model | updateStage = Updating }, updateUserInfo message )
        ...
        UpdateUserInfoSucceeded _ ->
            let
                ...
                newSubCont =
                    Content.updateAll model.inputContent.firstName model.inputContent.lastName model.inputContent.degree model.inputContent.terms subCont
            in
            ( { model | submittedContent = newSubCont, updateStage = Succeeded }, Cmd.none )
        ...
        TypedFirstName str ->
            let
                input =
                    Content.updateFirstName str model.inputContent
            in
        ...
        TypedLastName str ->
            let
                input =
                    Content.updateLastName str model.inputContent
            in
            ( { model | inputContent = input }, Cmd.none )
        ...
        CheckedBoxOne ->
            let
                ...
                newCheckedTerms =
                    ( not one, two, three )
            in
            ( { model | inputContent = input, checkedTerms = newCheckedTerms }, Cmd.none )
...
isLinkValid url =
    let
        decoder =
            Parser.s "verified" <?> Query.string "apiKey"
    in
...
hasChangedInfo model =
    let
        ...
        sub =
            model.submittedContent
    in
    inp.firstName /= sub.firstName || inp.lastName /= sub.lastName || inp.degree /= sub.degree || inp.terms /= sub.terms


infoIsNotEmpty : Model -> Bool
infoIsNotEmpty model =
    model.inputContent.firstName /= "" && model.inputContent.lastName /= "" && model.inputContent.degree /= None
...
showSubPage model =
    let
        msgToUser =
            Error.toString model.error

        miscDegree =
            model.user.degree == Valid MISC
